...
import { Building2, BookOpen, Wifi, Home, Layout, Cpu, Globe, Users, ShieldCheck, Zap, MonitorPlay, CheckCircle, GraduationCap, Trophy, Medal, Award, Star, Laptop, Phone, Mail, MapPin, Clock } from "lucide-react";
...
const facilities = [
    {
        ...
        gradient: "from-amber-500/20 to-amber-500/20",
...
export default function InfrastructureSection({ data, blocks }: { data?: any, blocks?: any[] }) {
    ...
    const displayFacilities = (blocks && blocks.length > 0) ? blocks.map((b: any) => {
        ...
        return {
            ...
            gradient: extra.gradient || "from-amber-500/20 to-amber-500/20",
            iconColor: extra.iconColor || "text-primary",
            iconName: extra.icon || 'Building2'
        };
    }) : [
        { title: "Adaptive Labs", description: "High-performance computational workspaces architected for intensive engineering tasks.", gradient: "from-amber-500/20 to-amber-500/20", iconColor: "text-primary", iconName: 'Building2' },
        { title: "Gigabit Mesh", description: "Enterprise-grade connectivity fabric ensuring zero-latency access to global knowledge.", gradient: "from-amber-500/20 to-amber-500/20", iconColor: "text-primary", iconName: 'Globe' },
