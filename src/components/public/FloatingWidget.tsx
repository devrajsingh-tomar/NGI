...
export default function FloatingWidget({ settings }: FloatingWidgetProps) {
    ...
    if (!settings?.floatingWidget?.enabled) return null;

    const { type, value, tooltipText = "Contact us" } = settings.floatingWidget;
    ...
    let icon = <LinkIcon className="w-5 h-5" />;
    ...
    if (type === "whatsapp") {
        ...
        icon = <Send className="w-5 h-5 rotate-[320deg] -translate-x-0.5 translate-y-0.5" />;
    ...
    return (
        <div 
            className="fixed right-4 top-1/2 -translate-y-1/2 z-50 flex items-center gap-3 pointer-events-none select-none"
        ...
        >
            ...
            <div 
                className={cn(
                    "px-4 py-2 bg-slate-900/95 backdrop-blur-sm text-white text-xs font-black rounded-xl border border-white/10 shadow-lg",
                    "transition-all duration-300 transform translate-x-2 opacity-0 origin-right whitespace-nowrap",
                    hovered && "translate-x-0 opacity-100"
                )}
            >
                {tooltipText}
            </div>
            ...
            <div className="relative pointer-events-auto">
                ...
                <div className={cn(
                    "absolute -inset-1 rounded-full border-2 animate-pulse opacity-30",
                    ringColor
                )} />
                <a 
                    href={url}
                    ...
                    className={cn(
                        "w-11 h-11 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95 shadow-xl hover:rotate-6",
                        themeColor
                    )}
                >
                    {icon}
