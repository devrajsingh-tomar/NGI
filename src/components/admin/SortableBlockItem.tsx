...
import { 
    ...
    Library, ShieldCheck, BookOpen, Globe, MonitorPlay, 
    CheckCircle, GraduationCap, Zap, Trophy, Medal, 
    Award, Star, Users, Laptop, Phone, Mail, MapPin, 
    Building2, Wifi, Cpu, Palette
} from "lucide-react";
...
export function SortableBlockItem({ block, index, onUpdate, onSave, onDelete, sectionType }: any) {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        formData.append("title", `Asset: ${block.title || "Untitled"}`);
    ...
    // Parse extra_data for custom sections
    const extraData = typeof block.extra_data === 'string' && block.extra_data ? JSON.parse(block.extra_data) : (block.extra_data || {});
    const updateExtra = (key: string, value: any) => {
        const newExtra = { ...extraData, [key]: value };
        onUpdate(block._id, "extra_data", JSON.stringify(newExtra));
    };
                ...
                <div className={cn("grid gap-8", needsMedia ? "md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2" : "grid-cols-1")}>
                    ...
                    <div className="space-y-6">
                        <div className="space-y-4">
                            {sectionType === "TrustIndicators" ? (
                            ...
                            ) : sectionType === "WhyChooseSection" || sectionType === "InfrastructureSection" ? (
                                <div className="space-y-4">
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Display Icon</label>
                                        <div className="flex flex-wrap gap-2 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                                            {iconOptions.map((opt) => (
                                                <button
                                                    key={opt.name}
                                                    type="button"
                                                    onClick={() => updateExtra("icon", opt.name)}
                                                    className={cn(
                                                        "p-2.5 rounded-xl transition-all",
                                                        extraData.icon === opt.name ? "bg-primary text-white shadow-lg" : "bg-white text-slate-400 hover:text-slate-900"
                                                    )}
                                                >
                                    ...
                                    {sectionType === "InfrastructureSection" && (
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Accent Color Palette</label>
                                            <div className="grid grid-cols-2 gap-3">
                                                {[
                                                    { label: "Amber (Default)", val: "from-amber-500/20 to-amber-500/20", color: "text-primary" },
                                                    { label: "Emerald (Success)", val: "from-emerald-500/20 to-teal-500/20", color: "text-emerald-600" },
                                                    { label: "Orange (Legacy)", val: "from-orange-500/20 to-amber-500/20", color: "text-orange-600" },
                                                    { label: "Slate (Neutral)", val: "from-slate-200 to-slate-300", color: "text-slate-600" }
                                                ].map((p) => (
                                                    <button
                                                        key={p.val}
                                                        type="button"
                                                        onClick={() => {
                                                            updateExtra("gradient", p.val);
                                                            updateExtra("iconColor", p.color);
                                                        }}
                                                        className={cn(
                                                            "flex items-center gap-3 p-3 rounded-xl border transition-all text-xs font-bold",
                                                            extraData.gradient === p.val ? "border-primary bg-orange-50 text-primary" : "border-slate-100 bg-white text-slate-500"
                                                        )}
                                                    >
                                                        <div className={cn("w-4 h-4 rounded-full bg-gradient-to-br", p.val)} />
                                                        {p.label}
                                                    </button>
                                                ))}
                                            </div>
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <Input 
                                            placeholder="E.g. Trusted IT Institute"
                                            className="h-12 rounded-xl bg-slate-50 border-none font-extrabold text-slate-900 text-base" 
                                            value={block.title || ""} 
                                            onChange={(e) => onUpdate(block._id, "title", e.target.value)} 
                                        />
                                    </div>
                                ...
                                <>
                                    <div className="space-y-4 mb-4">
                                        <div className="flex items-center gap-2 mb-2 pb-2 border-b border-slate-100">
                                            ...
                                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Access Points (Contact Info)</span>
                                        </div>
                                        <div className="space-y-1.5">
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <Input 
                                                placeholder="e.g. info@ngistudyzone.com"
                                                className="h-10 rounded-xl bg-slate-50 border-none font-bold text-sm text-slate-900 focus-visible:ring-1 focus-visible:ring-blue-500" 
                                                value={extraData.email || ""} 
                                                onChange={(e) => updateExtra("email", e.target.value)} 
                                            />
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Physical Address</label>
                                            <Textarea 
                                                placeholder="e.g. 123, Tech Corridor, Delhi"
                                                className="h-20 rounded-xl bg-slate-50 border-none font-bold text-sm text-slate-900 focus-visible:ring-1 focus-visible:ring-blue-500 resize-none" 
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-2 mb-2 pb-2 border-b border-slate-100">
                                            ...
                                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Operational Grid (Timings)</span>
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Monday - Friday</label>
                                            <Input 
                                                placeholder="e.g. 09:00 - 18:00"
                                                className="h-10 rounded-xl bg-slate-50 border-none font-bold text-sm text-slate-900 focus-visible:ring-1 focus-visible:ring-blue-500" 
                                                value={extraData.timing_mon_fri || ""} 
                                                onChange={(e) => updateExtra("timing_mon_fri", e.target.value)} 
                                            />
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Saturday</label>
                                            <Input 
                                                placeholder="e.g. 09:00 - 16:00"
                                                className="h-10 rounded-xl bg-slate-50 border-none font-bold text-sm text-slate-900 focus-visible:ring-1 focus-visible:ring-blue-500" 
                                                value={extraData.timing_sat || ""} 
                                                onChange={(e) => updateExtra("timing_sat", e.target.value)} 
                                            />
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Sunday</label>
                                            <Input 
                                                placeholder="e.g. Closed / Systems Offline"
                                                className="h-10 rounded-xl bg-slate-50 border-none font-bold text-sm text-slate-900 focus-visible:ring-1 focus-visible:ring-blue-500" 
                                                value={extraData.timing_sun || ""} 
                                                onChange={(e) => updateExtra("timing_sun", e.target.value)} 
                                            />
                                        </div>
                                    </div>
                                </>
                                ...
                                <>
                                    <div className="space-y-1.5">
                                        ...
                                        <Input 
                                            placeholder="E.g., Welcome to NGI Study Zone"
                                    ...
                                    {sectionType === "AboutSection" && (
                                        <div className="space-y-1.5 mt-4 p-4 bg-orange-50/50 border border-amber-100 rounded-[1.5rem]">
                                            <div className="flex items-center gap-2 mb-2 pb-2 border-b border-amber-200/50">
                                                <div className="w-1.5 h-1.5 rounded-full bg-amber-500" />
                                                <span className="text-[10px] font-black text-amber-700 uppercase tracking-widest">About Section Highlights</span>
                                            </div>
                                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Legacy Years (e.g. 15+)</label>
                                            <Input 
                                                placeholder="e.g. 15+"
                                                className="h-10 rounded-xl bg-white border-none font-bold text-sm text-slate-900 focus-visible:ring-1 focus-visible:ring-blue-500" 
                                                value={extraData.legacy_years || ""} 
                                                onChange={(e) => updateExtra("legacy_years", e.target.value)} 
                                            />
                                        </div>
                                    )}
                                </>
                            )}
                            {sectionType !== "TrustIndicators" && sectionType !== "ContactSection" && !["CourseGrid", "FacultyGrid"].includes(sectionType) && (
