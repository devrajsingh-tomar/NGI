    ...
    <aside className="w-56 bg-white border-r border-slate-100 h-full flex flex-col shrink-0">
      <div className="p-4 border-b border-slate-100">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-amber-600 to-indigo-600 flex items-center justify-center">
      ...
      <nav className="flex-1 p-3 overflow-y-auto space-y-0.5">
        {cmsNav.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={cn("w-3.5 h-3.5 shrink-0", active ? "text-amber-600" : "text-slate-400")} />
              {item.label}
            </Link>
          );
        })}
...
function QuickAction({ label, icon: Icon, href, color }: {
  ...
  return (
    ...
    >
      ...
      <span className="text-xs font-bold text-slate-600 group-hover:text-orange-600 text-center leading-tight">{label}</span>
    </Link>
  );
}
    ...
    <div className="flex flex-col h-full bg-[#f5f7fb]">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-4 py-2 text-xs font-medium bg-slate-50 border border-slate-200 rounded-lg w-52 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-400"
            />
          </div>
          ...
          <Link href="/admin/blogs/new" className="flex items-center gap-1.5 px-3 py-2 bg-amber-600 hover:bg-orange-600 text-white text-xs font-bold rounded-lg transition-all shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <StatCard label="Total Pages" value="12" icon={FileText} color="bg-orange-50 text-amber-600" />
          ...
          <StatCard label="Media Files" value="218" icon={ImageIcon} color="bg-orange-50 text-amber-600" />
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3">
            <QuickAction label="Edit Homepage" href="/admin/content/home-builder" icon={Layers} color="bg-orange-100 text-amber-600" />
            <QuickAction label="New Blog Post" href="/admin/blogs/new" icon={PenLine} color="bg-emerald-100 text-emerald-600" />
            <QuickAction label="Upload Media" href="/admin/gallery" icon={UploadCloud} color="bg-orange-100 text-amber-600" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              ...
              <Link href="/admin/content/pages" className="text-xs font-bold text-amber-600 hover:text-orange-700 flex items-center gap-1">
              ...
              {recentPages.map((page) => (
                <div key={page.slug} className="px-5 py-3 flex items-center justify-between hover:bg-slate-50/50 transition-colors group">
                  ...
                  <div className="flex items-center gap-3 ml-3 shrink-0">
                    ...
                    <Link href={`/admin/content/pages`} className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-orange-50 text-amber-600 transition-all">
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              ...
              <Link href="/admin/blogs" className="text-xs font-bold text-amber-600 hover:text-orange-700 flex items-center gap-1">
                View All <ChevronRight className="w-3 h-3" />
              </Link>
            </div>
            <div className="divide-y divide-slate-100">
              {recentBlogs.map((blog) => (
                <div key={blog.title} className="px-5 py-3 flex items-center justify-between hover:bg-slate-50/50 transition-colors group">
                  ...
                  <div className="flex items-center gap-3 ml-3 shrink-0">
                    ...
                    <Link href="/admin/blogs" className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-orange-50 text-amber-600 transition-all">
                      <PenLine className="w-3.5 h-3.5" />
                    </Link>
                  </div>
                </div>
              ...
              <div className="px-5 py-3">
                <Link href="/admin/blogs/new" className="flex items-center gap-2 text-xs font-bold text-amber-600 hover:text-orange-700 transition-colors">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-3">
            {[
              { label: "Homepage Builder", desc: "Hero, banners, sections", href: "/admin/content/home-builder", icon: Layers, color: "text-amber-600 bg-orange-50" },
              { label: "All Pages", desc: "Create & manage pages", href: "/admin/content/pages", icon: FileText, color: "text-amber-600 bg-orange-50" },
              ...
              <Link
                key={mod.href}
                href={mod.href}
                className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:border-orange-200 hover:shadow-md transition-all group flex flex-col"
              >
                ...
                <div className="mt-3 flex items-center text-[10px] font-black text-amber-600 opacity-0 group-hover:opacity-100 transition-opacity uppercase tracking-wide gap-1">
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="flex items-center gap-3 p-3 bg-orange-50 rounded-xl">
              ...
              <div>
                <p className="text-xs font-black text-orange-700">2 Scheduled</p>
                <p className="text-[10px] text-amber-600">Will publish soon</p>
              </div>
            </div>
