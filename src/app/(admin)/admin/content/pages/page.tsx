...
import { SectionMetadataEditor } from "@/components/admin/SectionMetadataEditor";
        ...
        <div className="space-y-8 max-w-full px-6 mx-auto pb-20 animate-in fade-in duration-1000">
            <div className="flex justify-between items-center px-4">
                 <div className="space-y-1">
                    ...
                    <p className="text-slate-400 font-black uppercase tracking-[0.3em] text-[10px]">Production-Grade CMS Architecture / NGI Study Zone V.4</p>
                </div>
                ...
                {sidebarVisible && (
                    ...
                    <div className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-100">
                        <div className="flex items-center justify-between mb-6 px-2">
                            ...
                            <Button size="icon" variant="ghost" onClick={handleCreatePage} className="h-9 w-9 hover:bg-orange-50 rounded-xl"><Plus className="w-5 h-5 text-amber-600" /></Button>
                        </div>
                        <div className="space-y-2">
                            {visiblePages.map(page => (
                                ...
                                >
                                    ...
                                    {activePageId === page._id && <ChevronRight className="w-4 h-4 text-amber-400" />}
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-100">
                        <div className="flex items-center justify-between mb-6 px-2">
                            ...
                            <Button size="icon" variant="ghost" onClick={handleCreateSection} className="h-9 w-9 hover:bg-orange-50 rounded-xl"><Plus className="w-5 h-5 text-amber-600" /></Button>
                        </div>
                            ...
                            {sections.map((section, idx) => (
                                <div key={section._id} className="group/item relative">
                                    <button 
                                        ...
                                        className={cn(
                                            ...
                                            activeSectionId === section._id 
                                                ? "border-amber-500 bg-orange-50/50 shadow-inner ring-4 ring-amber-500/5 rotate-1" 
                                                : "border-slate-100 hover:border-slate-300"
                                        )}
                                    >
                                        <div className="flex flex-col text-left">
                                            <span className="font-black text-[10px] text-slate-900 uppercase tracking-widest">{section.section_name}</span>
                                            <span className="text-[8px] uppercase font-bold text-amber-400 tracking-tighter mt-0.5">{section.section_type}</span>
                                        </div>
                                    </button>
                                    <div className="absolute -right-3 top-1/2 -translate-y-1/2 flex flex-col opacity-0 group-hover/item:opacity-100 transition-all z-20">
                                        <button onClick={() => handleMoveSection(section._id, "up")} className="p-1.5 bg-white border rounded-full mb-1 hover:text-amber-600 shadow-sm"><ArrowUp className="w-3 h-3" /></button>
                                        <button onClick={() => handleMoveSection(section._id, "down")} className="p-1.5 bg-white border rounded-full hover:text-amber-600 shadow-sm"><ArrowDown className="w-3 h-3" /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                                ...
                                {DATA_DRIVEN_SECTIONS[activeSection.section_type] ? (
                                    <div className="max-w-2xl mx-auto py-32 text-center space-y-10">
                                        <div className="relative inline-block">
                                            <div className="w-32 h-32 bg-orange-50 text-amber-600 rounded-[2.5rem] flex items-center justify-center mx-auto shadow-2xl border border-white">
                                    ...
                                    <div className="space-y-12">
                                        <HeroBlockEditor 
                                            ...
                                            initialSectionData={activeSection}
                        ...
                        <div className="flex-1 flex items-center justify-center p-20 text-center animate-in zoom-in-95 duration-1000">
                            <div className="max-w-sm space-y-8">
                                <div className="w-24 h-24 bg-white rounded-[2rem] shadow-2xl flex items-center justify-center mx-auto border border-slate-50 relative">
                                    ...
                                    <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-amber-500 rounded-full blur-xl opacity-20" />
                                </div>
                                <div className="space-y-3">
