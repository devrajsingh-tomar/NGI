...
export default async function PublicContactPage() {
    const [dynamicData, layoutData] = await Promise.all([
        getDynamicPageData("contact"),
        getHeaderFooterData()
    ]);

    const cmsSections = dynamicData.success && dynamicData.sections ? dynamicData.sections : [];
    const footer = layoutData.success ? layoutData.footer : null;

    // Extract contact info from footer sections
    const supportSection = footer?.sections?.find((s: any) => s.title.toLowerCase().includes('support') || s.title.toLowerCase().includes('contact'));
    const phoneNumbers = supportSection?.links?.filter((l: any) => l.href.startsWith('tel:')).map((l: any) => l.label.replace('Phone: ', '')) || ["+91 9369980633", "+91 9919303047"];
    const emails = supportSection?.links?.filter((l: any) => l.href.startsWith('mailto:')).map((l: any) => l.label.replace('Email: ', '')) || ["Olpsacadmy035@gmail.com", "Contact@nationalgeniusinstitute.com"];
    const address = supportSection?.links?.find((l: any) => !l.href.startsWith('tel:') && !l.href.startsWith('mailto:'))?.label || "NGI COMPUTER INSTITUTE SALORI PRAYAGRAJ, Chandpur Salori (Near Bahadur Chai) Prayagraj";
    const mapEmbed = footer?.mapEmbed || "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3601.5762928460044!2d81.87340689999999!3d25.48582!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x399ab531fe1e89e1%3A0xc96ab59adb557ac0!2sNational%20Genius%20Institute!5e0!3m2!1sen!2sin!4v1778533252331!5m2!1sen!2sin";
        ...
        <div className="min-h-screen bg-slate-50 pt-40 pb-24 overflow-hidden relative">
            ...
            <div className="container-custom relative z-10">
                ...
                <div className="text-center max-w-4xl mx-auto mb-24 space-y-8">
                    ...
                    <h1 className="text-6xl md:text-8xl font-black text-slate-900 leading-[0.9] tracking-tighter italic uppercase">
                ...
                <div className="grid lg:grid-cols-2 gap-20 items-start max-w-7xl mx-auto">
                                ...
                                <div className="flex items-start gap-8 group/item">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-col">
                                            {phoneNumbers.map((n: string, idx: number) => (
                                                <a key={idx} href={`tel:${n.replace(/\s+/g, '')}`} className="text-2xl font-black text-slate-900 hover:text-primary transition-colors tracking-tight">{n}</a>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-col gap-1">
                                            {emails.map((e: string, idx: number) => (
                                                <a key={idx} href={`mailto:${e}`} className="text-lg md:text-xl font-black text-slate-900 hover:text-indigo-600 transition-colors tracking-tight break-all">{e}</a>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="bg-white p-14 md:p-20 rounded-[5rem] shadow-2xl border border-slate-100 relative overflow-hidden">
                        ...
                        <h3 className="text-4xl font-black text-slate-900 mb-12 tracking-tighter italic leading-none">
                            Broadcasting <br /> <span className="text-slate-400 not-italic uppercase tracking-tighter leading-none">System</span>
                        </h3>
                        ...
                        <div className="space-y-10">
                            ...
                            <div className="p-8 bg-slate-50 rounded-[3rem] border border-slate-100 text-center">
                                ...
                                <p className="text-2xl font-black text-slate-900 tracking-tight italic">{phoneNumbers[0]}</p>
                            </div>
                        </div>
                    </div>
                </div>
